...
#from sklearn.mixture import bayesian_mixture # BayesianGaussianMixture

from sklearn.mixture import GaussianMixture # BayesianGaussianMixture

#import sklearn.mixture.BayesianGaussianMixture
...
#            g = {"mean":Xi_mean,"cov":Xi_cov}
            #gmm = bayesian_mixture(10) # 10 possible variations
            g = GaussianMixture(5)
            g.fit(Xi)
            print("iteration %d - Gaussian mixture fit complete",i)
    ...
    def sample_given_y(self,y):
         g = self.gaussians[y]
         sample = g.sample()
         # sample returns two items:
         # 1. the sameple 
         # 2. the culster to which belongs
         
         mean = g.means_[sample[1]]

         return self.clamp(sample[0].reshape(28,28)),mean.reshape(28,28) 
...
#        plt.show()
    ...
    for k in range(bayes_classifier.K):
        sample, mean = bayes_classifier.sample_given_y(k)
